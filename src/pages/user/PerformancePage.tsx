...
import { 
  ...
  Award, Activity, Calendar, Download, RefreshCw
} from 'lucide-react';
...
export default function PerformanceAnalyticsPage() {
  ...
  const fetchPerformanceData = async () => {
    setLoading(true);
    try {
      const response = await fetch(`/api/v1/trades/performance?range=${timeRange}`, {
        headers: {
      ...
      setData({
        ...
        bySymbol: [
          { symbol: 'XAUUSD', trades: 20, winRate: 70, profit: 1200 },
          { symbol: 'EURUSD', trades: 15, winRate: 60, profit: 800 },
          { symbol: 'GBPUSD', trades: 10, winRate: 70, profit: 450.75 }
        ],
        ...
        recentPerformance: Array.from({length: 30}, (_, i) => ({
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-gray-900 dark:text-white">
                  {Math.round(data.overview.totalTrades * data.overview.winRate / 100)}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-gray-900 dark:text-white">
                  {data.overview.totalTrades - Math.round(data.overview.totalTrades * data.overview.winRate / 100)}
                </span>
              </div>
            </div>
          </div>
      ...
      <Card>
        <div className="p-6">
          ...
          <div className="h-64 flex items-end gap-1">
            {data.recentPerformance.slice(-30).map((day, index) => {
              const maxProfit = Math.max(...data.recentPerformance.map(d => Math.abs(d.profit)));
